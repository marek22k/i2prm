...
$outport = rand(2000..2050)
$inport = rand(2000..2050)
while $outport == $inport
  $inport = rand(2000..2050)
end
...
$i2cpsettings["i2cp.leaseSetEncType"] = "4,0"
...
class MsgWindow < Fox::FXMainWindow
  ...
  def initialize app
    ...
    reconnectButton = FXButton.new listFrame, "Reconnect"
  ...
  def reconnectHandler
    Thread.new {
      begin
        ...
        sock = TCPSocket.new "127.0.0.1", $inport
        sock.puts $b64ss[$current]
        ans = sock.gets.chomp
        if ans[0..7] != "Hi. I am"
        ...
        remname = ans[10..-20]
        if remname != $current
          sock.puts "Our reconnect attempt does not work with this nickname. The desired nickname is #{$current}."
          sock.close
          Thread.current.exit
        end
        ...
        clifc = TCPSocket.new "127.0.0.1", $inport
        clifc.puts $b64ss[$current]
...
class OptionsWindow < Fox::FXMainWindow
  ...
  def initialize app
    ...
    app.addSignal("SIGINT") {
      $sigintcounter += 1
      if $sigintcounter < 3
        connectHandler connectButton, nicknameBox, b64Box if $statusLabel.text == "Status: Connected"
        exit
      else
        exit!
      end
  ...
  def connectHandler myself, nicknameBox, b64Box
    if myself.text == "Connect"
      ...
      $serv = Thread.new do
        ...
        $consoleLabel.text = $cli.gets.chomp
        $cli.puts "outport #{$outport}"
        $consoleLabel.text = $cli.gets.chomp
        $cli.puts "inhost 127.0.0.1"
        $consoleLabel.text = $cli.gets.chomp
        $cli.puts "inport #{$inport}"
        $consoleLabel.text = $cli.gets.chomp
        $cli.puts "start"
        $consoleLabel.text = $cli.gets.chomp
        ...
        serv = TCPServer.new "127.0.0.1", $outport
        loop do
